...
class DetectorTests(unittest.TestCase):
    ...
    def test_comparison_uses_filenames_as_columns(self):
        ...
        self.assertRegex(report, r"(?m)^\[\^1\]: Différence entre texte IA")
        ...
        self.assertNotIn("| Amplitude (caractères)[^10] |", summary)
        self.assertIn("| Amplitude (caractères)[^10] |", details)
        self.assertNotIn("| Mots-outils[^17] |", summary)
        self.assertIn("| Mots-outils[^17] |", details)
        self.assertNotIn("| Noms[^18] |", summary)
        self.assertIn("| Noms[^18] |", details)
        self.assertNotIn("| Verbes[^19] |", summary)
        self.assertIn("| Verbes[^19] |", details)
        self.assertNotIn("| Adjectifs[^20] |", summary)
        self.assertIn("| Adjectifs[^20] |", details)
        self.assertNotIn("| Adverbes[^21] |", summary)
        self.assertIn("| Adverbes[^21] |", details)
        ...
        self.assertNotIn("| Rythme des structures[^22] |", summary)
        self.assertIn("| Rythme des structures[^22] |", details)
        self.assertNotIn("| Compression gzip[^23] |", summary)
        self.assertIn("| Compression gzip[^23] |", details)
        self.assertIn("| Relatives et subordonnées[^5] |", summary)
        self.assertRegex(summary, r"\| Relatives et subordonnées\[\^5\] \| (?:\d+ %|indisponible)")
        self.assertRegex(summary, r"\| Relatives et subordonnées\[\^5\] \| .* \| \d+,\d % \|")
        self.assertIn("| Ponctuation (signes/300 mots)[^6] |", summary)
        self.assertIn("| Diversité de ponctuation[^7] |", summary)
        self.assertIn("| Variété des débuts de phrase[^9] |", summary)
        ...
        self.assertNotIn("| Profondeur syntaxique[^24] |", summary)
        self.assertIn("| Profondeur syntaxique[^24] |", details)
        ...
        self.assertIn("| Variation des phrases (mots)[^11] |", report)
        self.assertIn("| Longueur moyenne des phrases (mots) |", report)
        self.assertIn("| Diversité des formes[^25] |", report)
        self.assertIn("| Diversité lemmatisée[^26] |", report)
        self.assertIn("| Mots employés une seule fois[^27] |", report)
        self.assertIn("| Répétition globale des trigrammes[^28] |", report)
        self.assertIn("| Répétition locale des trigrammes[^29] |", report)
        self.assertIn("| Taux de répétition non filtré[^30] |", report)
        self.assertIn("Répétition des structures[^3]", report)
        self.assertIn("| Diversité de ponctuation[^7] |", report)
        self.assertIn("| Variété des débuts de phrase[^9] |", report)
        self.assertNotIn("Plus longue série de phrases proches", report)
        self.assertIn("| Noms[^18] |", report)
        self.assertIn("| Verbes[^19] |", report)
        self.assertIn("| Adjectifs[^20] |", report)
        self.assertRegex(report, r"\| Compression gzip\[\^23\] \| .* \| \d+,\d % \|")
        self.assertRegex(report, r"\| Mots \| .* \| — \|")
        self.assertGreater(details.index("| Mots |"), details.index("| Taux de répétition non filtré[^30] |"))
        self.assertGreater(details.index("| Fenêtres de répétition analysées |"), details.index("| Taux de répétition non filtré[^30] |"))
